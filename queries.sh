...
echo  "$($PSQL "select name from teams as t inner join games as g on t.team_id = g.winner_id where g.round = 'Final' and year = 2018")"
...
echo  "$($PSQL "select year, name from games as g inner join teams as t on g.winner_id = t.team_id where round = 'Final' order by year asc")";
